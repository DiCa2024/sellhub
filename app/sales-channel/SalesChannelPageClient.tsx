      ...
      <div className="mx-auto max-w-7xl">
            ...
            <div className="rounded-[24px] border border-neutral-200 bg-neutral-50 p-4 md:p-5">
              <div className="flex flex-col gap-3 md:flex-row">
                <input
                  ...
                  placeholder="채널명, 카테고리, 정산일, 설명으로 검색"
          ...
          <div className="mb-8 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center justify-between">
              <div className="text-sm text-neutral-700">
                비교함에{" "}
                <span className="font-semibold">{compareIds.length}</span>개
                담겼어요.
        ...
        <section>
          <div className="mb-4 text-sm text-neutral-600">
            검색 결과{" "}
            <span className="font-semibold">{filteredChannels.length}</span>개
          </div>
...
function RecommendSection({
  ...
  return (
    <section className="mt-16">
      ...
      <div className="grid gap-6 md:grid-cols-4">
        {items.map((item) => {
          const titleText = type === "blog" ? item.title : item.name;
          const link =
            type === "blog" ? `/blog/${item.id}` : `/wholesale/${item.id}`;
          const fallback =
            type === "blog"
