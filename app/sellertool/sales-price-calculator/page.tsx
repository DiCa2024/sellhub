...
export default function SalesPriceCalculatorPage() {
  ...
  const baseCost = purchase + shipping + extra;

  const sellingPrice =
    commission < 100
      ? baseCost / (1 - commission / 100 - margin / 100)
      : 0;

  const commissionFee = (sellingPrice * commission) / 100;
  const profit = sellingPrice - (baseCost + commissionFee);
  ...
  return (
    <main className="min-h-[calc(100vh-80px)] bg-neutral-50 px-6 py-10">
      <div className="mx-auto max-w-3xl rounded-2xl border bg-white p-8 shadow-sm">
        <h1 className="text-3xl font-bold">판매가 계산기</h1>
        <p className="mt-2 text-sm text-neutral-600">
          목표 마진율을 기준으로 적정 판매가를 자동 계산합니다.
        </p>
     <div className="mt-4 flex gap-2">
      ...
      <a href="/sellertool/commission-calculator" className="rounded-lg border px-3 py-2 text-sm hover:bg-neutral-100">
        ...
        <div className="mt-8 grid gap-4">
          ...
          <Input label="목표 마진율 (%)" value={targetMargin} onChange={setTargetMargin} />
        </div>
        ...
        <div className="mt-10 rounded-2xl border bg-neutral-50 p-6">
          ...
          <div className="mt-4 space-y-2 text-sm">
            <Result label="추천 판매가" value={`${Math.round(sellingPrice).toLocaleString()}원`} />
            <Result label="수수료 금액" value={`${Math.round(commissionFee).toLocaleString()}원`} />
            <Result label="예상 순이익" value={`${Math.round(profit).toLocaleString()}원`} />
          </div>
        </div>
